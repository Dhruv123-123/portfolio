  ...
  <div class="ab-root bb-scroll">
    ...
    <div class="ab-col">
      ...
      <ul>
        ...
        <li><b>FlightGear</b>: every replay exports as a package for the open-source simulator (positions at 10 Hz in FlightGear's generic protocol anchored on the crash-site coordinates, the protocol file, an fgfs command line with aircraft, date, time and weather from the record). With FlightGear running with --httpd=8080 and an external flight model, the browser replay drives it live over the property WebSocket, so it flies the same instant over real terrain.</li>
        <li><b>Cockpit track</b>: for replays with a transcript, each line is spoken offline with a voice per seat and language and each warning is synthesized to the recorded flags, then scheduled on the audio clock. Nothing in it is a real recording; the real tapes are the separate, labelled layer.</li>
        <li><b>Replay fidelity</b>: six replays are digitised from published plots; the rest were reconstructed by workers from the report text ("narrative") or from the record's timed states ("schematic"), and an audit removes any file whose record holds fewer than four altitude or speed points. The label sits next to the record chip.</li>
